...
export function generateSystemPrompt(config: PromptConfig = DEFAULT_CONFIG): string {
  ...
  const selectedReviewers = config.reviewers || [reverse]

  let levelItems = USER_LEVEL_DESCRIPTIONS['apprentice'].map((level) => `- ${level}`).join('\n');

  // Build focus area descriptions based on selected reviewers
  let focusItems = '';
  if (selectedReviewers.length === 1) {

    // eg ['general', 'performance']
    const focusKeys = config.reviewers.map(r => r.focus);

    // eg ['general', 'performance']
    const focusItems = focusKeys.map(key => FOCUS_AREA_DESCRIPTIONS[key]).join('\n');

  } else {
    // Multiple reviewers - combine their focus areas
    const allFocusAreas = selectedReviewers.flatMap(reviewer => FOCUS_AREA_DESCRIPTIONS[reviewer.focus]);
